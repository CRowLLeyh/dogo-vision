...
export const mockPlayerData = {
  ...
  // Base matches (we generate 20 from these to keep mockData compact)
  recentMatches: (() => {
    const base: MatchData[] = [
      ...
      },
      {
        ...
        blueTeam: [
          ...
          { summonerName: "OriannaMid", champion: "Orianna", championIcon: "https://ddragon.leoflegends.com/cdn/14.1.1/img/champion/Orianna.png", kills: 7, deaths: 2, assists: 10, rank: "Plat III", damage: 24800 },
    ...
    // Generate exactly 20 "most recent" matches for the UI
    const makeTimeAgo = (i: number) => {
      if (i < 10) return `${(i + 2) * 2} horas atrás`;
      if (i < 15) return `${i - 8} dias atrás`;
      return `${i - 13} semanas atrás`;
    };

    return Array.from({ length: 20 }, (_, i) => {
      const b = base[i % base.length];
      const isMvp = i === 0 || i === 11;
      ...
      return {
        ...b,
        matchId: `BR1_${123456 + i}`,
        timeAgo: makeTimeAgo(i),
        largestMultikill: Math.max(b.largestMultikill, multikill),
        isMvp: isMvp ? true : undefined,
      } as MatchData;
    });
  })(),
  topChampions: [
